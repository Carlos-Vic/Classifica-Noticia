...
st.write('Filtros:')
portal = st.selectbox(label='Portal',options=['Todos'] + list(portais.keys()), index=0, on_change=reseta_pagina)
vies = st.selectbox(label='Viés', options=['Todos', 'Direita', 'Esquerda'], index=0, on_change=reseta_pagina)

if 'pagina' not in st.session_state:  
     st.session_state['pagina'] = 1 
    

offset = (st.session_state['pagina'] - 1) * 10
